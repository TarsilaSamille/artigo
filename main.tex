...
The rapid advancement of Natural Language Processing (NLP), driven by the Transformer architecture~\cite{vaswani2017attention}, has paradoxically widened the gap between global ``high-resource'' and ``low-resource'' languages~\cite{nekoto2020participatory}. Foundational models such as GPT-4 and Gemini are predominantly trained on web-scraped datasets, where English, Mandarin, Spanish, and standard Malay account for the vast majority of linguistic tokens~\cite{nllb2022no}.
...
The fundamental separation between local and remote computational workloads is mapped in Figure \ref{fig:arch}. This \textbf{Hybrid Decentralization} strategy splits the RAG pipeline into two distinct security and compute zones:
\begin{itemize}

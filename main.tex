...
\lstset{
    ...
    commentstyle=\color{black!60!black}\itshape,
...
The rapid progress in Natural Language Processing (NLP), driven by the Transformer architecture~\cite{vaswani2017attention}, has disproportionately benefited global ``high-resource'' languages, leaving ``low-resource'' languages further behind~\cite{nekoto2020participatory}. Foundational models such as GPT-4 and Gemini are predominantly trained on web-scraped datasets, where English, Mandarin, Spanish, and standard Malay account for the vast majority of linguistic tokens~\cite{nllb2022no}.

This resource disparity is especially problematic for Bidayuh Jagoy (also referred to under the broader ethno-linguistic cluster of Land Dayak), a distinct dialect spoken primarily in the West Kalimantan region of Indonesia and parts of Sarawak, Malaysia. While spoken widely in the community, it has a very limited digital footprint. Its presence is confined to scattered PDFs, academic papers from institutions like Universiti Malaysia Sarawak (UNIMAS), and community blogs~\cite{unimas_repo}. Consequently, when standard LLMs attempt to process Bidayuh Jagoy, they frequently fail, producing hallucinations, code-switching to standard Malay, and grammatical errors~\cite{scispace_intensifiers}.

Retrieval-Augmented Generation (RAG)~\cite{intersystems_rag} offers a practical alternative for this scenario. This architecture distinguishes between two types of knowledge: \textbf{Implicit Memory}, which is baked into the model's internal parameters during pre-training but often contains hallucinations for low-resource data, and \textbf{Explicit Memory}, which consists of external context information provided to the model at runtime. The Bidayuh Jagoy Translator uses RAG to bypass the limits of implicit memory, assembling a structured bunch of helper information (vocabulary, sentence patterns, and grammar rules) to instruct the Google Gemini model from scratch for each query.
...
The fundamental separation between local and remote computational workloads is mapped in Figure \ref{fig:arch}. This hybrid decentralization strategy splits the RAG pipeline into two distinct security and compute zones:
\begin{itemize}
    \item The client layer runs directly in the user's browser, securely managing the AI logic, local glossary state, and generating embeddings for re-ranking. Keeping these operations local ensures that user queries remain private and eliminates the latency of remote embedding calls.
    \item The cloud infrastructure consists of a serverless backend (Vercel and Supabase) that persists the global parallel corpus and handles high-recall retrieval using pgvector. This allows the application to search the central repository efficiently without forcing users to download the entire dataset.
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{UI.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{analysis.png}
\caption{Translation Analysis Trace}
\label{fig:analysis}
\end{figure}

\begin{enumerate}
    \item \textbf{Context A (Local Glossary Lookups):} Executes direct queries against a local JSON dictionary. For our example, it maps \textit{stone} $\rightarrow$ \textit{batuh}. This provides 1:1 lexical grounding, preventing ``lexical drift'' where the LLM might hallucinate standard Malay (\textit{batu}).
    \item \textbf{Context B (Hybrid Vector-Keyword Retrieval):} Queries the remote Supabase HNSW index \cite{supabase_hnsw} while simultaneously performing a local keyword search. It retrieves semantically similar sentence pairs (e.g., \textit{``picked up stones''}) to suggest proper SVO orientation.
...
This complete 5-stage sequential assembly pattern is visually structured in Figure \ref{fig:pipeline}. To support linguistic auditing, evaluators can expand a \textbf{``Full Prompt (Debug)''} section in the UI to view the exact retrieved contexts from A-E sent to Gemini.


\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig_pipeline_highres.png}
...
General-purpose foundation models cannot translate Bidayuh Jagoy natively due to data scarcity. The system addresses this by guiding the LLM to follow an algorithmic ``Chain-of-Thought'' (CoT) \cite{wei2022chain}. The instructions strictly mandate a 3-step reasoning block based on the retrieved contexts:
\begin{enumerate}
...
To handle this high-dimensional context, the system utilizes \textbf{Google Gemini 2.5 Flash}, selected for its 1-million-token window \cite{data_studios_gemini}. The feasibility of this structured synthesis is further detailed in Section \ref{sec:fat_client}, which discusses the local execution model. An explicit breakdown of this reasoning prompt is provided in listing \ref{lst:reasoning}.

\begin{lstlisting}[language=Java, basicstyle=\footnotesize, frame=single, breaklines=true, caption={Reasoning Chain}, label={lst:reasoning}]
...
\begin{enumerate}
    ...
    \item \textbf{Heuristic Refinement}: We implemented symmetric alignment checking, retaining only bidirectional hits.
...
A robust evaluation of translation quality in low-resource settings requires native speaker validation. The Bidaio Jagoy Translator implements an integrated \textit{History and Correction Module}. As shown in the system interface in image \ref{fig:history}, every translation is logged with a unique identifier and presented to users for manual verification. This mechanism allows the community to:
\begin{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{history.png}
\caption{Translation History Module}
\label{fig:history}
\end{figure}
...
The Bidayuh Jagoy Translator combines neural search with traditional rule-based linguistics. By leveraging Transformers.js, the system makes advanced RAG tools accessible for endangered language preservation, decoupling it from expensive server-side GPU clusters.
